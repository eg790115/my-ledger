...
const LoginUI = ({
  ...
  // 🛡️ 終極防呆裝甲：保證底下的畫面不管拿到什麼爛資料，都絕對不會崩潰
  const safePinInput = pinInput || "";
  const safeFamilyConfig = familyConfig || [];
      ...
      {!selectingUser ? (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 w-full max-w-xs text-white animate-in">
            {safeFamilyConfig.map(user => ( 
              <button key={user?.name || Math.random()} onClick={() => handleUserClick(user)} className={`bg-${user?.color ? user.color.replace('bg-', '') : (user?.name==="媽媽"?"pink-600":"blue-600")} py-6 rounded-[2.5rem] font-black text-2xl shadow-lg border-4 border-white/5 active:scale-95 transition-all text-white`}>{user?.name || "未知成員"}</button> 
            ))}
          </div>
        ...
        <div className="w-full max-w-sm animate-in text-white text-center font-black">
          <div className="bg-gray-800 rounded-[2.5rem] p-8 shadow-2xl border border-white/10 relative">
            <div className={`bg-${selectingUser?.color ? selectingUser.color.replace('bg-', '') : (selectingUser?.name==="媽媽"?"pink-600":"blue-600")} w-16 h-16 mx-auto rounded-[1.5rem] flex items-center justify-center text-white font-black text-2xl shadow-lg mb-4`}>{selectingUser?.name?.charAt(0) || "U"}</div>
            <p className="font-black text-3xl mb-1 tracking-tight text-white">{selectingUser?.name || "未知"}</p>
            ...
            {isDeviceBioBound(selectingUser?.name) && !fallbackToPin ? (
              ...
              <>
                <div className="flex justify-center gap-3 mb-10">
                  {/* 🛡️ 這裡原本直接讀取 pinInput.length，現在換成了 safePinInput，徹底根絕報錯 */}
                  {[...Array(6)].map((_, i) => <div key={i} className={`w-3.5 h-3.5 rounded-full border-2 border-blue-500 transition-all duration-200 ${safePinInput.length > i ? "bg-blue-500 scale-125 shadow-[0_0_12px_#3b82f6]" : "bg-transparent"}`}></div>)}
                </div>
