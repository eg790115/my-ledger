...
const HistoryTab = ({
  ...
  const safeStats = historyFilteredStats || { income: 0, expense: 0 };
  ...
  return (
    <div className="flex flex-col animate-in w-full relative text-left">
      
      {/* 🌟 頂部過濾區與垃圾桶 */}
      <div className="flex justify-between items-center mb-4 gap-2 shrink-0 px-1">
      ...
      {/* 🌟 統計面板：只保留總收入與總支出，並美化 */}
      <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm mb-4 flex gap-3 shrink-0 mx-1">
        <div className="flex-1 bg-green-50/50 rounded-[1.25rem] p-3 flex items-center gap-3 border border-green-100/50 transition-all hover:bg-green-50">
          <div className="w-10 h-10 rounded-xl bg-green-100 text-green-600 flex flex-col items-center justify-center font-black text-lg shrink-0">收</div>
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-black text-green-600/70 mb-0.5">總收入</div>
            <div className="text-base font-black text-green-700 truncate tracking-tight">${safeStats.income.toLocaleString()}</div>
          </div>
        </div>
        <div className="flex-1 bg-red-50/50 rounded-[1.25rem] p-3 flex items-center gap-3 border border-red-100/50 transition-all hover:bg-red-50">
          <div className="w-10 h-10 rounded-xl bg-red-100 text-red-500 flex flex-col items-center justify-center font-black text-lg shrink-0">支</div>
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-black text-red-500/70 mb-0.5">總支出</div>
            <div className="text-base font-black text-red-600 truncate tracking-tight">${safeStats.expense.toLocaleString()}</div>
          </div>
        </div>
      </div>

      {/* 🌟 歷史清單：Virtuoso 虛擬列表引擎 */}
      <div className="flex-1 w-full pb-24 mt-2">
        {safeGroups.length === 0 ? (
