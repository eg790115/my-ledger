...
function App() {
  ...
  const handleConfirmVoice = () => {
    ...
    const completeTxs = (voiceReviewTxs || []).map((tx, idx) => {
      ...
      return {
         ...
         date: (tx.date || nowStr()).replace('T', ' '), type: tx.type || "expense",
  ...
  const historyFilteredStats = useMemo(() => {
    ...
    (filteredHistoryGroups || []).forEach(item => {
      if (item.isGroup) { (item.children || []).forEach(c => { if (c.type === "income") inc += (Number(c.amount) || 0); if (c.type === "expense") exp += (Number(c.amount) || 0); }); } 
      else { if (item.type === "income") inc += (Number(item.amount) || 0); if (item.type === "expense") exp += (Number(item.amount) || 0); }
    });
  ...
  return (
    <div translate="no" className="min-h-screen bg-gray-50 text-gray-900 pb-24 max-w-md mx-auto relative flex flex-col overflow-x-hidden w-full text-left font-black">
      ...
      {editingGroup && ( <EditGroupParentModal group={editingGroup} onSave={(g) => handleUpdateGroupParent(g)} onCancel={() => { if(triggerVibration) triggerVibration(10); setEditingGroup(null); }} /> )}
        ...
        <div className="fixed inset-0 z-[600] bg-gray-900/90 backdrop-blur-sm overflow-y-auto flex flex-col items-center justify-center p-4 sm:p-6 animate-in font-black" onClick={(e) => { if(e.target === e.currentTarget) setShowTrashModal(false); }}>
          <div className="bg-white w-full max-w-sm relative rounded-[2.5rem] shadow-2xl overflow-hidden pb-6 flex flex-col max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-2 space-y-3 scrollbar-hide">
              {(visibleTrash || []).length === 0 ? ( <div className="text-center text-gray-400 py-10 text-sm">回收桶是空的</div> ) : ( (visibleTrash || []).map(tx => (
                  <div key={tx.id} className="bg-white p-3 rounded-2xl border border-gray-100 flex flex-col gap-2 relative overflow-hidden">
                    <div className="flex justify-between items-start"><div><span className="text-xs font-black text-gray-800">{getParentCat(tx.category)} - {getChildCat(tx.category)}</span><div className="text-[10px] text-gray-400 mt-0.5">{displayDateClean(tx.date)}</div></div><span className={`font-black text-sm ${tx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>${Number(tx.amount).toLocaleString()}</span></div>
        ...
        <div className="fixed inset-0 z-[700] bg-gray-900/90 backdrop-blur-md flex flex-col items-center justify-center p-4 sm:p-6 animate-in font-black" onClick={(e) => { if(e.target === e.currentTarget) setViewingHistoryItem(null); }}>
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl relative flex flex-col max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
              {(() => {
                let logs = viewingHistoryItem.editHistory ? [...viewingHistoryItem.editHistory] : [];
                ...
                return logs.map((h, i) => {
                  ...
                  return (
                    <div key={i} className="bg-gray-50 p-4 rounded-2xl border border-gray-100 relative overflow-hidden">
                      ...
                      <div className="relative z-10 flex items-center gap-2 mb-2 text-xs font-bold text-gray-500"><div className="w-2 h-2 rounded-full bg-yellow-400 shadow-[0_0_0_3px_#FEF9C3]"></div>{h.time} <span className="text-gray-400">({h.recorder} 修改)</span></div>
                      <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm ml-4">
                        ...
                        {snapshot && snapshot['金額'] !== undefined ? ( <ul className="text-xs space-y-1.5 font-medium text-gray-600"><li className="flex justify-between"><span>💰 金額</span> <span className="font-black text-gray-800">${snapshot['金額']}</span></li><li className="flex justify-between"><span>🏷️ 類別</span> <span className="text-gray-800">{snapshot['類別']}</span></li><li className="flex justify-between"><span>📅 日期</span> <span className="text-gray-800">{snapshot['日期']}</span></li><li className="flex justify-between"><span>👤 出錢/對象</span> <span className="text-gray-800">{snapshot['成員']} ➔ {snapshot['對象']}</span></li>{snapshot['備註'] && ( <li className="pt-1 mt-1 border-t border-gray-50"><span className="block text-[10px] text-gray-400 mb-0.5">📝 備註</span><span className="block text-gray-800 bg-gray-50 p-1.5 rounded-lg">{snapshot['備註']}</span></li> )}</ul> ) : ( <div className="text-xs text-gray-500 italic">{snapshot ? snapshot['內容'] : '無詳細快照'}</div> )}
                      </div>
                    </div>
                  );
                });
